...
// get recent
        response = client.prepareSearch("emails").setSearchType(SearchType.COUNT).
                addAggregation(AggregationBuilders.filter("filter1m").filter(FilterBuilders.rangeFilter("submit_time").to("now").from("now-1M")).
                        subAggregation(AggregationBuilders.terms("stats").field("topic.raw").size(40).order(Terms.Order.aggregation("ta",true)).
    ...
    private Map<String,Long> pullAggsResult(Aggregation agg) {
        Map<String,Long> sm = new LinkedHashMap<>();
        if (agg!=null) {
            for (Terms.Bucket statBusket :((Terms) agg).getBuckets()) {
                sm.put(statBusket.getKey(),statBusket.getDocCount());
            }
        }
        return sm;
    }
